...
export const StoreDashboard: React.FC<StoreDashboardProps> = ({ user, onLogout }) => {
  ...
  const stats = [
    ...
    { label: 'Total Produtos', value: MOCK_PRODUCTS.filter(p => p.storeId === user.id).length, icon: <Package size={20} />, color: 'bg-emerald-500', lightColor: 'bg-emerald-50', textColor: 'text-emerald-600' },
  ...
  return (
    <div className="p-6 pb-32 space-y-8 max-w-lg mx-auto bg-slate-50 min-h-screen">
      ...
      <section className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Painel do Parceiro</h1>
        ...
        <div className="space-y-8">
          {[1, 2].map((i) => (
            <div key={i} className="flex gap-4 items-start">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900 leading-tight">Nova resposta à cotação #MQ{123+i}</p>
