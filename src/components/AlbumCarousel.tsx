        ...
        <div key={album.id} className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs font-medium text-primary/50">{album.description}</p>
            </div>
            <div className="flex gap-2">
              <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-background text-primary/40 active:scale-95">
                <ChevronLeft size={18} />
              </button>
              <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-background text-primary active:scale-95">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 snap-x snap-mandatory">
            {album.images.map((image, idx) => (
              <motion.div 
                key={idx}
                whileTap={{ scale: 0.98 }}
                className="relative h-64 w-80 flex-shrink-0 snap-center overflow-hidden rounded-xl bg-background shadow-sm"
              >
                <img 
                  src={image} 
